...
app.post('/customer', async (req, res) => {
    ...
    if (!enroll_date) { enroll_date = new Date(); }
    ...
    try {
        await withTransaction(client, async () => {
            const result = await client.query(`
                SELECT 
                    a.ID,
                    COALESCE(MAX(b.End_date), a.Signup_date) AS latest_date
                FROM plan AS a
        ...
        await withTransaction(client, async () => {
            const call_query = `
                CREATE TEMPORARY TABLE call_totals AS
                    ...
                    FROM PLAN AS p
                    JOIN PLAN_OPTION AS po ON p.Option = po.Option
                    JOIN CUSTOMER AS c ON c.Plan_ID = p.ID
                    LEFT OUTER JOIN CALL AS cl ON cl.Phone = c.Phone
                    WHERE cl.Billed = false
                    GROUP BY p.ID, EXTRACT(MONTH FROM cl.Start_time), po.C_rate, po.C_over_rate, po.C_limit
            `;
            ...
            const cost_calls_query = `
                CREATE TEMPORARY TABLE cost_of_calls AS
                    SELECT
                        ...
                        a.month_of_calls,
                        CASE
                            WHEN a.call_duration < a.C_limit THEN a.call_duration * a.C_rate * 1.08
